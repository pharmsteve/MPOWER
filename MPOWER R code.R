...
#2 Subgroup analysis of Countries with initially high and earnestly improved MPOWER scores
...
tidy(mod, conf.int = T)
mod <- plm(Smokeless_male ~ MPOWE + Price + GNI, data = earnest, index = c("Country", "Year"), model = "within", effect = "twoways")
tidy(mod, conf.int = T)
mod <- plm(Smokeless_female ~ MPOWE + Price + GNI, data = earnest, index = c("Country", "Year"), model = "within", effect = "twoways")
tidy(mod, conf.int = T)
...
mod <- plm(Smoked_male ~ MPOWE + R + GNI, data = pan, index = c("Country", "Year"), model = "within", effect = "twoways")
...
mod <- plm(Smoked_male ~ MPOWE + R + GNI, data = earnest, index = c("Country", "Year"), model = "within", effect = "twoways")
tidy(mod, conf.int = T)
